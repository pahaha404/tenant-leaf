...
- 2026-08-24 — 매물 탭 "새 매물 등록" 하단 가로 직사각형 버튼을 우측 하단 오렌지 원형 플로팅 액션 버튼(Circular FAB `+`)으로 변경. 리스트 스크롤 시 화면 하단 영역을 가리지 않고 쾌적하게 목록을 탐색할 수 있으며, 엄지손가락으로 손쉽게 탭하여 신규 매물을 추가할 수 있도록 직관적인 모바일 표준 FAB UX를 완성함. `:app:testDebugUnitTest` 158개 태스크 전원 통과 (`BUILD SUCCESSFUL`).
