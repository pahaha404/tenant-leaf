...
- 2026-08-26 — 리포트 관찰 카드의 `AI 신뢰도`와 근거 사진 BBOX 라벨의 퍼센트 표시를 제거하되 내부 신뢰도 데이터는 유지함. 매물 등록·수정 금액 입력 단위를 만원으로 바꾸고 관리비 입력·저장을 연결했으며, 한 줄로 저장된 숫자 동·호 상세 주소가 수정 폼에 복원되도록 보완함. 점검 음성 기록은 매물 상세에서 리포트 완료·부분 완료·빈 결과 화면으로 이동함. 주소 분리·만원 환산 테스트와 정적 검사를 추가했으나 오프라인 환경에서 Gradle 플러그인을 해석하지 못해 빌드·실기기 검증은 미완료 상태다.
