...
- 2026-08-24 — Android 리포트 UI를 완료·근거 사진·빈 결과·부분 완료·오류·생성 중 6개 상태로 구현하고 기존 리포트 생성 진행 플래그에 연결함. AI 결과는 `확인 필요 관찰`로 표현하고, 근거 사진의 정규화 bbox·신뢰도·구역을 전체화면에서 확인하도록 구성함. Gradle/AGP 배포본 다운로드 차단으로 빌드·테스트는 미검증이며 실제 리포트 API ViewModel 연결도 후속 작업으로 유지함.
