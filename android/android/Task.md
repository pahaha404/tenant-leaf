...
- 2026-08-24 — 실시간 점검 단조 증가 타임스탬프(Monotonic Synthetic PTS) 및 안경/스마트폰 듀얼 소스 일시정지/재개 엔진 보강. 기기 전환(안경 ↔ 폰)과 일시정지/재개 반복 시에도 타임스탬프 왜곡이나 프레임 드롭 없이 매 프레임 정확히 33,333us(30fps)씩 단조 증가하는 연속 스트림 타임라인을 보장하고, 스마트폰 카메라 일시정지/재개 누락을 해결함. `:app:testDebugUnitTest` 158개 태스크 전원 통과 (`BUILD SUCCESSFUL`). 실기기 갤러리 실제 비디오 인덱싱 및 재생은 실기기 미검증임.
