...
- 2026-08-26 — 매물 지도 화면(`PropertyMapOverviewScreen`)에서 매물 핀 선택 시 하단 요약 패널 카드 팝업 높이에 맞춰 플로팅 뒤로가기 버튼과 내 위치 이동 FAB의 오프셋 높이를 위쪽(`bottom = 250.dp`)으로 조정하여 패널과 전혀 겹치지 않도록 깔끔하게 수정 완료.

- 2026-08-26 — 매물 탭 화면에서 가로 구분선(`HorizontalDivider`)을 완전히 제거하여 한결 깔끔하고 여유로운 심플 모던 레이아웃으로 수정 완료.

- 2026-08-26 — 매물 삭제 모드(`isSelectionMode == true`) 시 가로 구분선(`HorizontalDivider`)과 선택 정보 로우("0개 선택됨 | 전체 선택") 사이의 불필요했던 빈 공백을 완전히 제거하여 구분선 바로 아래 군더더기 없이 깔끔하게 밀착 연동 완료.

- 2026-08-26 — 가로 구분선(`HorizontalDivider`)과 하단 매물 목록 카드 사이의 여백을 2배(`bottom = 14.dp`)로 확대하여 여유롭고 시원한 공백 레이아웃을 완성함.

- 2026-08-26 — 이전의 세로 구분선을 제거하고 상단 헤더("매물 목록" 타이틀 및 오른쪽 버튼)와 하단 매물 카드 목록 영역 사이에 깔끔한 가로 구분선(`HorizontalDivider`, `0xFFE5E7EB`, `1.dp`)을 추가 적용 완료.

- 2026-08-26 — 매물 탭 타이틀을 "매물 목록"으로 변경하고 옆에 1.2.dp 세로 구분선(`|`)을 추가함. 헤더 로우의 수직 정렬을 수직 중앙(`Alignment.CenterVertically`)으로 맞춰 일반 모드 듀얼 필 버튼(`[ 🗺️ | 🗑️ ]`)과 삭제 모드 닫기 버튼(`[ X ]`)이 모드 전환 시 위치 이동 없이 100% 동일한 Y축 상단 높이에 고정 배치되도록 수정 완료.

- 2026-08-26 — "어느 매물의 리포트를 볼까요?" 서브 타이틀 폰트 크기를 기존 20.sp에서 `16.5.sp`로 축소하여 단정하게 정돈하고, 카드 선택 상태 배경색을 이전보다 한 톤 더 또렷하게 어두운 쿨그레이(`Color(0xFFEDEFEF)`)로 변경 완료.

- 2026-08-26 — 리포트 탭 상단 타이틀("리포트 선택")과 "어느 매물의 리포트를 볼까요?" 본문 간 여백을 기존의 절반(1/2)으로 축소하고, 리포트 패널 카드 내 매물 이름과 주소 사이의 세로 공백도 1.dp로 좁혀 한눈에 들어오도록 정돈 완료.

- 2026-08-26 — 리포트 탭 및 매물 탭 카드의 선택 상태 배경색을 첨부 사진과 동일한 부드러운 소프트 그레이 톤(`Color(0xFFF8F9FA)`)으로 적용하고, 다크그린 테두리(`BorderStroke(1.5.dp, DeepGreen)`)는 동일하게 유지하도록 수정 완료.

- 2026-08-26 — 리포트 탭 매물 카드(`ReportPropertyCard`)의 기존 회색 아웃라인 테두리를 제거하고 매물 탭 카드와 동일한 3D 소프트 쉐도우(`1.5.dp`), 라운딩 둥근 모서리(`18.dp`), 선택 시 다크그린 테두리(`BorderStroke(1.5.dp, DeepGreen)`) 및 `PaleGreen` 배경 컨테이너 스타일로 통일 적용 완료.
