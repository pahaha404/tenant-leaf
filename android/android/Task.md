...
- 2026-08-24 — 실시간 점검 비디오 레코더(`InspectionVideoRecorder`) Android Media Best Practices 전면 개편. 코덱 EOS(`signalEndOfInputStream`) 시그널링, 스레드 안전성(`AtomicBoolean` 및 동기화 락), Lifecycle `onDispose` 안전 자원 회수, Android 10~15 Scoped Storage 4단계 정석 파이프라인(메타데이터 `WIDTH`/`HEIGHT`/`DURATION` 완비 및 `MediaScannerConnection` 동기 대기)을 구축함. `:app:testDebugUnitTest` 158개 태스크 전원 통과 (`BUILD SUCCESSFUL`). 실기기 갤러리 실제 비디오 인덱싱 및 재생은 실기기 미검증임.
