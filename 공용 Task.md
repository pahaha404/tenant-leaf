...
- 2026-08-25 — 리포트의 안전·하자 점수화 오해를 막기 위해 참고 점수를 공통 계약·OpenAPI·서버·Android에서 제거했다. Gemini가 사람 포함 여부를 응답하고 확실히 사람이 없는 JPEG만 리포트 대표 사진으로 선정하는 개인정보 보호 규칙을 추가했으며, Android 전송 완료 화면의 무한 로딩 표시를 수정했다. 외부 플러그인·Python 실행 환경 문제로 전체 빌드와 실제 Gemini·실기기 검증은 미완료다.

- 2026-08-25 — Gemini 공간 분류를 리포트 대표 사진 계약까지 연결해 주방·거실/방·화장실별로 사진을 묶고, 같은 공간에서 촬영 시점이 6초 미만인 사진은 중복 대표로 제외하도록 보완했다. 공간별 최대 3장·전체 최대 12장을 제공하고 불확실한 공간은 `공간 확인 필요`로 표시한다. 정적 검사와 OpenAPI 변경 검토는 완료했으나 현재 실행 환경에서 Gradle 플러그인을 오프라인 해석하지 못해 서버·Android 빌드와 실기기 확인은 미검증 상태다.

- 2026-08-25 — 리포트에서 임장 공간을 다시 확인할 수 있도록 대표 JPEG 계약·서버 선별·Android 사진 갤러리를 연결하고, 하자 관찰은 AI 공간명을 확정적으로 표시하지 않은 채 촬영 순서로 제공하도록 정리했다. 분석 진행 수치는 사진 단위로 순차 표시하도록 보완했다. 정적 검사는 통과했지만 현재 환경의 원격 Gradle 의존성 제한으로 전체 빌드·테스트와 실기기 확인은 미검증이므로 관련 완료 항목은 유지한다.

- 2026-08-25 — Android 임장 시작 시 기존 `3초 뒤 촬영이 시작됩니다` 안내 후 현관·거실·주방·화장실·창틀 점검 체크리스트를 TTS로 이어서 발화하도록 추가함. 화면의 `3→2→1` 카운트다운은 유지하되 숫자는 발화하지 않고, 각 안내 문장 사이에 2초 간격을 적용함. `:app:testDebugUnitTest`, `:app:assembleDebug`를 통과했으며 실기기 발화 순서·간격·음량은 미검증임.

- 2026-08-25 — Android 앱 시작 직후 뜨던 주변 기기 권한 요청을 `점검에 필요한 권한을 확인해 주세요` 화면 진입 시점으로 이동함. `촬영 내용을 분석하고 있어요` 화면은 Android 버전에 맞는 사진·동영상 권한을 요청하며 Android 14 선택 접근도 처리함. 두 화면 모두 미허용 권한만 요청하고 콜백 후 실제 승인 상태를 재확인하도록 보완했으며 `:app:testDebugUnitTest`, `:app:assembleDebug`를 통과함. 실기기 권한 팝업 위치는 미검증임.

- 2026-08-25 — Android 신규 사용자 흐름을 `온보딩 → 로그인 → 동의` 순서로 정리함. 온보딩 완료 상태를 저장해 재실행 시 건너뛰며 `initialRouteFor` 회귀 테스트를 보강함. 실제 화면 흐름은 실기기 미검증임.
