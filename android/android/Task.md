...
- 2026-08-26 — 마이페이지(`ProfileScreen`) 최종 통합 완성. 상단바 표준 44dp 수정 버튼(`topTrailingAction`), 럭셔리 에메랄드 히어로 프로필 카드, 3단 KPI 대시보드(내 매물 5개, 점검 리포트 3건, 안전 지수 98점)와 함께 **부동산 편의 도구 3종**(평수↔㎡ 면적 계산기, 복비 계산기, 전입신고 안전 가이드) 및 **앱 설정 및 환경**(알림/피드백 토글 3종, 촬영/미디어 환경 2종, 실제 파일시스템 바이트 측정 1-Tap 캐시 정리, FAQ, 약관 3종)을 유기적으로 결합 완료. `:app:testDebugUnitTest` 전원 통과 (`BUILD SUCCESSFUL`).

- 2026-08-26 — 마이페이지(`ProfileScreen`) 레이아웃 정돈 완료. 사용자 요청에 따라 1) 'Lv.1 스마트 세입자 · 카카오 계정 연동됨' 태그 및 2) '안심 점검 패스 VIP 활성화' 배너를 프로필 카드에서 제거하고 슬림한 미니멀 히어로 카드로 정돈함. 3) 'Ray-Ban Meta AI 글라스 카드' 및 4) '임장 안심 도구 & 가이드' 섹션을 화면에서 제거하여, 핵심 지표(내 매물·리포트·안전 지수)와 스마트 앱 환경 설정 및 계정 관리에 집중된 깔끔한 UI를 완성함. `:app:testDebugUnitTest` 전원 통과 (`BUILD SUCCESSFUL`).
...
- 2026-08-26 — 리포트 관찰 카드의 `AI 신뢰도`와 근거 사진 BBOX 라벨의 퍼센트 표시를 제거하되 내부 신뢰도 데이터는 유지함. 매물 등록·수정 금액 입력 단위를 만원으로 바꾸고 관리비 입력·저장을 연결했으며, 한 줄로 저장된 숫자 동·호 상세 주소가 수정 폼에 복원되도록 보완함. 점검 음성 기록은 매물 상세에서 리포트 완료·부분 완료·빈 결과 화면으로 이동함.

- 2026-08-26 — 점검 음성 기록에서 Android STT 변환·재시도·원문/요약 UI를 제거하고, 로컬 WAV 재생/일시정지·탐색 슬라이더·현재/전체 시간 표시를 종료 확인·매물 상세·음성 기록 화면에 공통 적용함.
