...
Android 앱, Kotlin API 서버와 AI 작업자가 같은 이름과 데이터 의미를 사용하기 위한 MVP 계약이다. 2.0은 현장 체크리스트 중심의 1.2 계약을 **구역·관찰·근거 미디어 중심**으로 교체했고, 2.1은 JPEG 업로드 계약을, 2.2는 미디어 집합 확정·AI 구역 분류·관찰 생성·자동 리포트 조회 계약을, 2.3은 리포트 분석 진행률의 전체 JPEG 수 계약을, 2.4는 AI 구역 분류 범위를 주방·거실/방·화장실과 미확정 구역으로 정리했다. 2.5는 리포트 대표 사진과 촬영 순서 기준 관찰 표시 계약을 추가한 검토 초안이다. 분석용 JPEG의 최대 크기는 사진당 2MiB다.
...
- 리포트 상세의 `representativePhotos`는 분석 완료 JPEG 중 임장 공간의 흐름을 다시 볼 수 있도록 선별한 만료형 조회 URL 목록이며, 관찰이 0건이어도 제공할 수 있다.
- 대표 사진은 촬영 시점 오름차순으로 반환한다. Android는 AI 구역명을 대표 사진의 확정 설명으로 표시하지 않고 전체화면 확대와 좌우 탐색을 제공한다.
