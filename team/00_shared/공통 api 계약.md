...
Android 앱, Kotlin API 서버와 AI 작업자가 같은 이름과 데이터 의미를 사용하기 위한 MVP 계약이다. 2.0은 현장 체크리스트 중심의 1.2 계약을 **구역·관찰·근거 미디어 중심**으로 교체했고, 2.1은 JPEG 업로드 계약을, 2.2는 미디어 집합 확정·AI 구역 분류·관찰 생성·자동 리포트 조회 계약을, 2.3은 리포트 분석 진행률의 전체 JPEG 수 계약을, 2.4는 AI 구역 분류 범위를 주방·거실/방·화장실과 미확정 구역으로 정리했다. 2.5는 리포트 대표 사진과 촬영 순서 기준 관찰 표시 계약을, 2.6은 Gemini 공간별 다중 대표 사진 계약을, 2.7은 참고 점수 폐지와 사람이 포함된 사진의 대표 제외 계약을 추가한 검토 초안이다. 분석용 JPEG의 최대 크기는 사진당 2MiB다.
...
- AI 정규 구역은 `KITCHEN`, `LIVING_ROOM`, `BATHROOM` 세 개다. 현관과 창틀은 AI 분류 범위에서 제외하며 해당 공간 또는 지원하지 않는 응답은 `UNKNOWN`으로 정규화한다.
- `UNKNOWN`은 세 개 정규 구역 완료 수에 포함하지 않는다.
...
- 리포트는 관찰 수와 근거를 제공하며 안전·하자·계약을 수치로 평가하는 참고 점수를 생성하거나 응답하지 않는다.
...
- 리포트 상세의 `representativePhotos`는 Gemini가 확실하게 분류한 `KITCHEN`, `LIVING_ROOM`, `BATHROOM`별 대표 JPEG와 불확실한 경우의 `UNKNOWN`을 제공하며 관찰이 0건이어도 제공할 수 있다.
- Android가 흔들림·밝기·직전 사진 중복 비교를 거쳐 업로드한 JPEG를 전제로 하고, 서버는 같은 공간에서 영상 시점 차가 6초 미만인 사진을 대표 목록에서 중복 선택하지 않는다. 공간별 최대 3장, 전체 최대 12장을 제공한다.
- Gemini가 사람·얼굴·식별 가능한 신체 일부가 포함됐다고 판별한 JPEG는 리포트 대표 사진에서 제외한다. 판별값이 없거나 호출이 실패한 경우도 보수적으로 대표 후보에서 제외한다.
- Android는 `zoneUncertain=false`인 대표 사진만 `주방 대표 사진`, `거실·방 대표 사진`, `화장실 대표 사진`처럼 Gemini 분류 결과로 묶어 표시한다. `UNKNOWN` 또는 `zoneUncertain=true`는 `공간 확인 필요`로 표시하고, 전체화면 확대와 좌우 탐색을 제공한다.
- 확인 필요 관찰은 잘못된 공간 분류를 사용자에게 확정적으로 전달하지 않도록 대표 근거의 `sourceVideoOffsetMs` 오름차순으로 표시한다. MVP 리포트에서 관찰을 AI 구역명으로 묶어 표시하지 않는다.
...
| 2026-08-25 | 검토 초안 2.5 | 리포트 대표 사진 목록, 관찰 0건 대표 사진 제공, 전체화면 좌우 탐색과 촬영 순서 기준 관찰 표시 계약 추가 |
| 2026-08-25 | 검토 초안 2.6 | Gemini의 주방·거실/방·화장실 분류값과 불확실 여부를 대표 사진 응답에 포함하고 공간별 다중 대표 사진 선별 규칙 추가 |
| 2026-08-25 | 검토 초안 2.7 | 참고 점수 API·UI·생성 규칙 폐지, Gemini 사람 포함 판별값 미확인 사진의 대표 선정 제외 |
