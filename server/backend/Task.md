...
- 2026-08-25 — 모바일 데이터 임시 시연을 위해 API와 MinIO를 각각 Cloudflare Quick Tunnel HTTPS 주소로 노출하고, API가 외부 MinIO 주소로 presigned URL을 만들도록 재시작하는 스크립트를 추가했다. 외부 API·MinIO health 응답은 확인했으나, 실제 휴대전화 모바일 데이터에서 JPEG PUT과 리포트까지 이어지는 전체 검증은 미완료 상태다.
...
- 2026-08-25 — 리포트 참고 점수를 OpenAPI·서버 응답·생성 로직·DB 스키마에서 제거했다. Gemini 공간 분류에 사람 포함 판별을 추가하고 `contains_person=false`가 확인된 JPEG만 대표 사진 후보로 선정하도록 Worker·DB·선별 규칙을 연결했다. 정적 검사는 통과했으나 현재 환경의 Kotlin `2.3.21` 플러그인 오프라인 해석 실패와 Python 가상환경 기준 실행 파일 유실로 전체 테스트는 미완료다.
