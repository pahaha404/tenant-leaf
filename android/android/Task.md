...
- 2026-08-25 — 게스트 로그인 성공 시 `Consent`로 고정 이동해 신규 사용자의 3단 온보딩을 건너뛰던 문제를 수정함. 최초 진입은 로그인 화면에서 시작하고, 로그인 후 온보딩 미완료 상태면 `Welcome`을 거쳐 동의 화면으로 이동하며 완료 상태면 바로 동의 화면으로 이동함. `initialRouteFor` 회귀 테스트를 보강하고 `:app:testDebugUnitTest`를 통과함. 실기기 화면 흐름은 미검증임.
