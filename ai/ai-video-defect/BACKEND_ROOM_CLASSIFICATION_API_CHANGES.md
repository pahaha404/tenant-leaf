...
- Python 3.11 또는 3.12
- 프로젝트 전용 가상환경
- GPU 사용 시 서버 CUDA와 호환되는 PyTorch·torchvision
- CPU 서버라면 CPU용 PyTorch·torchvision

`requirements.txt`의 직접 설치 항목은 다음과 같다. `google-auth`, `httpx`, `pydantic` 등은 `google-genai`의 의존성이므로 별도로 버전을 관리하지 않는다.
...
PyTorch 공식 설치 명령으로 `torch`, `torchvision`을 먼저 설치한 뒤 나머지 패키지를 설치한다.
...
# 서버 CUDA 또는 CPU에 맞는 torch, torchvision을 먼저 설치
python -m pip install -r requirements.txt
```

Windows PowerShell에서는 가상환경 활성화 명령만 다음과 같이 바꾼다.

```powershell
.\.venv\Scripts\Activate.ps1
```

### 6-2. Gemini 모델과 API 키

기본 공간 분류 모델은 `gemini-3.5-flash-lite`다. 기존 `gemini-2.5-flash-lite`는 신규 프로젝트에서 `404 NOT_FOUND`가 발생할 수 있으므로 사용하지 않는다.

`GEMINI_API_KEY`는 Python AI worker가 실제로 실행되는 프로세스 환경에 비밀변수로 설정한다. 터미널에만 설정하면 systemd, Docker 또는 별도 queue worker에는 전달되지 않을 수 있다. 변수 변경 후 worker를 재시작한다.
...
API 키는 요청 JSON, manifest, DB, 로그, Git 저장소 또는 사용자 응답에 넣지 않는다. 기존에 노출된 키가 있다면 폐기하고 새 키를 발급한다.

### 6-3. 네트워크 조건

- AI worker에서 `generativelanguage.googleapis.com`으로 나가는 HTTPS 443 연결을 허용한다.
- Gemini 사용을 위해 외부에 새 인바운드 포트를 열 필요는 없다.
- 프록시·방화벽 환경에서는 해당 도메인의 DNS 조회와 TLS 연결을 확인한다.
- 공간 분류 대상 이미지는 Google Gemini API로 전송되므로 테스트·운영 데이터 정책과 사용자 동의 범위를 확인한다.

### 6-4. 설치 사전 점검

API 키 값을 화면에 출력하지 말고 설정 여부만 확인한다.
...
| 증상 | 원인 | 조치 |
|---|---|---|
| `did not find executable` | 다른 PC·Python 경로로 만든 가상환경을 복사했거나 원본 Python 제거 | `.venv`를 서버에서 새로 만들고 의존성 재설치 |
| `No module named 'google'` | worker가 다른 Python을 사용하거나 SDK 미설치 | worker와 같은 가상환경에서 `python -m pip install -r requirements.txt` 실행 |
| `404 NOT_FOUND`와 2.5 모델명 | 오래된 모델 기본값 | 배포 파일을 갱신하고 `gemini-3.5-flash-lite` 사용 |
| `GEMINI_API_KEY is required` | worker 프로세스에 키가 전달되지 않음 | 서비스·컨테이너 비밀변수 설정 후 worker 재시작 |
| timeout·connection 오류 | DNS, 프록시 또는 방화벽 차단 | Google API 도메인 outbound HTTPS 443 확인 |
| CUDA 관련 오류 | PyTorch 빌드와 서버 CUDA 불일치 | 서버 환경에 맞는 PyTorch 재설치 또는 CPU 모드 확인 |
...
AI worker 실행 환경에는 `GEMINI_API_KEY` 비밀 환경변수가 필요하다.
...
- [ ] 기본 모델이 `gemini-3.5-flash-lite`인지 확인
- [ ] Google Gemini API outbound HTTPS 443 연결 확인
- [ ] API 키 변경 후 worker 재시작 및 키 비노출 확인
