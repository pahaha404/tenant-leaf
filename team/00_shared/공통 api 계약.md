...
feature/api-observation-generation
> 상태: 검토 초안 2.4
>
> 기준일: 2026-08-25
...
> 도메인 기준: [`도메인 규칙.md`](../../server/backendmds/도메인%20규칙.md)
 ...
 feature/api-observation-generation
Android 앱, Kotlin API 서버와 AI 작업자가 같은 이름과 데이터 의미를 사용하기 위한 MVP 계약이다. 2.0은 현장 체크리스트 중심의 1.2 계약을 **구역·관찰·근거 미디어 중심**으로 교체했고, 2.1은 JPEG 업로드 계약을, 2.2는 미디어 집합 확정·AI 구역 분류·관찰 생성·자동 리포트 조회 계약을, 2.3은 리포트 분석 진행률의 전체 JPEG 수 계약을, 2.4는 AI 구역 분류 범위를 주방·거실/방·화장실과 미확정 구역으로 정리한 검토 초안이다. 분석용 JPEG의 최대 크기는 사진당 2MiB다.
