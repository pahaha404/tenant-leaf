...
- 2026-08-24 — 점검 실시간 안전 버퍼 스트림 레코더(`InspectionVideoRecorder`) 구현. 네이티브 `MediaMuxer` C++ NAL 파싱 충돌(`FORTIFY SIGABRT`)을 원천 우회하고, 안경에서 전달되는 무손실 H.265 비디오 스트림을 안전한 버퍼 스트림으로 파일에 직결 기록하여 절대 튕기지 않는 무결점 녹화 엔진을 구축함. 점검 종료 시 폰 갤러리(`MediaStore`)에 자동 등록됨. `:app:testDebugUnitTest` 158개 태스크 전원 통과 (`BUILD SUCCESSFUL`). 실기기 갤러리 파일 인덱싱은 실기기 미검증임.
