...
- 2026-08-24 — 실시간 점검 표준 H.264 MP4 Surface 인코더 및 Scoped Storage 갤러리 등록 엔진(`InspectionVideoRecorder`) 전면 개편. 뷰파인더 `TextureView` 프레임을 안드로이드 표준 `MediaCodec` Surface Encoder + `MediaMuxer`로 30fps 정석 인코딩하여 `ftyp/moov` 완비 표준 MP4 파일 생성. Android 10~15 Scoped Storage `MediaStore.Video.Media` 스트림 복사 및 `MediaScannerConnection` 인덱싱 완료 후 화면을 전환하도록 레이스 컨디션을 해결함. `:app:testDebugUnitTest` 158개 태스크 전원 통과 (`BUILD SUCCESSFUL`). 실기기 갤러리 실제 비디오 인덱싱 및 재생은 실기기 미검증임.
