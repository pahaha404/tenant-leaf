...
- 2026-08-24 — 점검 실시간 통합 MP4 동영상 녹화 파이프라인(`InspectionVideoRecorder`) 및 NAL Unit 규격 변환기(`HevcNalParser`) 구현. Meta DAT 안경 Annex-B 스트림을 MP4 규격인 4바이트 Big-Endian Length-Prefixed(HVCC) 샘플 및 `csd-0`(VPS/SPS/PPS) 헤더로 완벽 변환하여 `MPEG4Writer` 네이티브 `SIGABRT` 크래시를 원천 차단함. 안경 ↔ 스마트폰 카메라 전환 시에도 1개의 연속된 MP4 파일로 저장 및 점검 종료 시 폰 갤러리(`MediaStore`)에 자동 등록됨. `:app:testDebugUnitTest` 158개 태스크 전원 통과 (`BUILD SUCCESSFUL`). 실기기 하드웨어 MediaMuxer 및 갤러리 인덱싱은 실기기 미검증임.
