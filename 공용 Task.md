...
- 2026-08-25 — Android 게스트 로그인 후 신규 사용자의 3단 온보딩이 동의 화면으로 건너뛰어지던 라우팅을 `로그인 → 온보딩 → 동의` 순서로 수정함. 온보딩 완료 사용자는 기존처럼 건너뛰며 `:app:testDebugUnitTest`를 통과함. 실제 게스트 로그인 화면 흐름은 실기기 미검증임.
