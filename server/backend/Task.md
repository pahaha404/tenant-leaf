...
- 2026-08-24 — 분석 완료 JPEG의 원시 탐지를 사용자 관찰로 변환하고 자동 리포트를 집계하는 DB V8·Spring Boot API·주기적 조정 작업을 구현함. 관찰별 근거와 동일 사진의 다중 픽셀 `xyxy` bbox, 짧은 만료 조회 URL, 잠정 참고 점수를 응답하도록 연결했으나 Gradle 플러그인 원격 해석 제한으로 서버 컴파일·통합 테스트는 미검증이므로 관련 체크 항목은 `[ ]`로 유지함.
